...
describe('Testa o componente Header', () => {
  ...
  test('Testa se o botão de saiba mais possui o link correto para nova página', () => {
    ...
    expect(button1).toHaveAttribute('href', '/product-detail/0');

    const button2 = screen.getByTestId('button-component-link-1');

    expect(button2).toHaveAttribute('href', '/product-detail/1');
  });
  test('Testa se ao clicar no botão de saiba mais, é redirecionado para página de detalhes', () => {
    const history = createMemoryHistory();
    const button1 = screen.getByTestId('button-component-link-0');
    const href = button1.getAttribute('href');
    userEvent.click(button1);
    history.push(href);

    const url = history.location.pathname;
    expect(url).toBe('/product-detail/0');
  });
